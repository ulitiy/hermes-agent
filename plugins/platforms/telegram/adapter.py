...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Allows trusted local processes (e.g. iphone_relay.py) to
            # inject synthetic user messages into this Telegram session
            # so they land in the same in-memory conversation as real
            # Telegram messages. Bound to 127.0.0.1, gated by shared
            # secret. Silently no-op if HERMES_TELEGRAM_INJECT_SECRET
            # is not set.
            try:
    ...
    async def disconnect(self) -> None:
        ...
        # HERMES-LOCAL PATCH: tear down injection endpoint
